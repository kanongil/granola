...
const test = function (obj) {
    ...
    const json = CS.stringify(obj);
    console.log('json:', json);

    const parsed = JSON.parse(json);
    console.log('parsed', parsed);

    const doc = CS.parse(parsed);

    console.log('output:', doc);
    if (Hoek.deepEqual(obj, doc)) {
        console.log('***passed***');
    }
};
...
//test({ $x: {} });
//test({ $x: {}, '@': 123 });
//test({ $x: Buffer.from('a') });
//test(Buffer.from('3e39c403407257f245f490ed08149ada', 'hex'));
test({ a: Buffer.from('a') });
test({ a: [123, Buffer.from('a')] });
test({ a: { b: Buffer.from('b') }, c: true });
//test([{ a: { b: Buffer.from('b') }, c: true }]);
...
try {
    new Function('test', `
        ...
        test({ a: { b: 10n }, c: true });
        //test([{ a: { b: 10n }, c: true }]);
        test({ a: new Set([false, 1n, "2", 3, new Map([[1n, 'a']])]), b: { c: new Map([[1n, 'a']]) } });
        test({ a: new Map([[1n, 'a']]) });
    `)(test);
}
catch (err) { }
